...
function uostype_entity_event_dragstart(event) {
  ...
  for(var i=0; i<elementdata.filetypes.length; i++) {
  	if (elementdata.filetypes[i].mimetype) {
			event.dataTransfer.setData(elementdata.filetypes[i].mimetype, elementdata.filetypes[i].url);  
			uos.log(elementdata.filetypes[i].mimetype,elementdata.filetypes[i].url);
		}
  }
