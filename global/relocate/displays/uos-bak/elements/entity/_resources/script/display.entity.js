...
function uostype_entity_event_drop(event) {
  ...
  var data = event.dataTransfer.getData('text');
		...
		case 'application/uos' :
	  	if (!uos.isSelected($element)) {
	    	...
	    	uos.post(elementdata.guid,'dropentity',{
	    		dropcontent: uos.getSelectedGuids()
	    	})
	    	uos.log(elementdata.guid);
	    }  
		break;
    ...
    // if we drop content
	    ...
	    var contenttrimmed = (content.length>100) ? content.substring(0, 100) + '...' : content;
...
function uostype_entity_identify_content(event) {
	uos.log('uostype_entity_identifycontent : ' + event.dataTransfer.types[0] ); 
